...
public:
	...
	static ptr SigScan(const char* pattern, const char* target = "ShadowOfWar.exe")
	{
	...
	static ptr SigScan(const char* pattern, std::uintptr_t start)
	{
	...
	static ptr GetCallAddress(const char* pattern, const char* module_name)
	{
		auto address = Memory::SigScan(pattern, module_name);
		if (!address)
		{
			return 0;
		}
